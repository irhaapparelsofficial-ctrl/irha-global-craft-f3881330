...
function json(value, status = 200, extra = {}) {
  return new Response(JSON.stringify(value), { status, headers: responseHeaders(extra) });
}
...
function rpcError(id, code, message, reason, metadata) {
  return { jsonrpc: "2.0", id: id ?? null, error: { code, message, data: errorData(reason, metadata) } };
}
...
function buyerAnswer(text) {
  ...
  if (/\b(admin|crm|lead export|bulk email|social post|publish|owner dashboard|private file)\b/.test(lower)) {
    return "The public Irha Apparels A2A agent cannot access owner-only admin, CRM, outreach, social publishing or private-file systems. Those actions require the authenticated owner dashboard and human approval.";
  }
  if (/\b(factory|video call|verification|verify|buyer trust|audit)\b/.test(lower)) {
    return `Irha Apparels is an experienced manufacturer and the current website is newly built.\nBuyer verification: ${SITE_ORIGIN}/buyer-trust\nLive factory-view video call: ${SITE_ORIGIN}/factory-video-call\nNo meeting has been booked by this response.`;
  }
  ...
  if (/\b(product|collection|lederhosen|dirndl|trachten|leather|sportswear|activewear|streetwear|nightwear|uniform)\b/.test(lower)) {
    return `Irha Apparels manufactures made-to-order B2B apparel for brands, importers, wholesalers and private-label buyers.\nBavarian Heritage & Leather: ${SITE_ORIGIN}/products/bavarian-trachten-wear\nAll public manufacturing programs: ${SITE_ORIGIN}/products\nRequest a reviewed quotation or catalogue: ${SITE_ORIGIN}/inquiry`;
  }
  ...
  return {
    ...
    result: {
      message: {
        ...
        contextId: safeText(message.contextId, 200) || crypto.randomUUID(),
        role: "ROLE_AGENT",
        parts: [{ text: buyerAnswer(parsed.text), mediaType: "text/plain" }],
        metadata: { readOnly: true, submitted: false, source: "irha-public-a2a", clientMessageId },
      },
    },
  };
}
