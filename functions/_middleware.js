...
const PAGE_SUMMARIES = {
  ...
  "/inquiry": { title: "B2B Inquiry", summary: "Buyer form for RFQs, catalogue requests, samples, mockups, references and meeting requests. A request is sent only after the buyer reviews and submits the form." },
...
function markdownFor(pathname) {
  ...
  const productPage = normalized.startsWith("/products/")
    ? { title: "Irha Apparels Product Program", summary: "Public B2B product and manufacturing information. Product feasibility, materials, MOQ, price, timing, shipping and documents are confirmed after human review." }
    : null;
  const cataloguePage = normalized.startsWith("/catalogue/")
    ? { title: "Irha Apparels Catalogue Group", summary: "Public B2B catalogue information. Use the inquiry page to request a quotation or product-specific confirmation." }
    : null;
  const blogPage = normalized.startsWith("/blog/")
    ? { title: "Irha Apparels Article", summary: "Public manufacturing and buyer education content." }
    : null;
  const page = exact || productPage || cataloguePage || blogPage || { title: "Irha Apparels", summary: "Public B2B apparel manufacturing information from Irha Apparels." };
