...
begin
  ...
  insert into private.catalog_media_change_audit(
    ...
    'classic-bavarian-checkered-shirt-four-view-20260717',
    target_id,
    before_json,
    after_json
  )
  ...
  set image_url = front_url,
      gallery = array[front_url, three_quarter_url, side_url, back_url]::text[],
      updated_at = now()
  where id = target_id
    and (
      image_url is distinct from front_url
      or gallery is distinct from array[front_url, three_quarter_url, side_url, back_url]::text[]
    );
end
$migration$;
