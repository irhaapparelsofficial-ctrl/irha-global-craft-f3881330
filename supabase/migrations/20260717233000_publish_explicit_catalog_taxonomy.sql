...
-- This publishes only the already reviewed Main Category -> Audience/Buyer Group
-- -> Product Type structure. Existing category rows, product rows and legacy URLs
-- are preserved. Redirects remain approved but are not applied by this migration.
...
begin
  ...
  if exists (
    select 1 from public.catalog_taxonomy_nodes where publish_state <> 'published'
  ) then
    raise exception 'taxonomy publication was incomplete';
  end if;
  if exists (
    select 1 from public.product_taxonomy_assignments where review_state <> 'approved'
  ) then
    raise exception 'taxonomy assignment approval was incomplete';
  end if;
  if exists (
    select 1 from public.catalog_taxonomy_migration_map where redirect_status = 'applied'
  ) then
