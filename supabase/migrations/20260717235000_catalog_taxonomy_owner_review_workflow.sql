...
-- Owner-driven catalogue taxonomy release workflow.
-- Applying this migration never approves assignments or publishes taxonomy nodes.
-- The owner must first complete the existing one-by-one hierarchy and product
-- assignment review. Publication then requires an exact confirmation phrase,
-- current-count preconditions and an immutable hierarchy/mapping snapshot.
...
begin
  ...
  with metrics as (
    select
      ...
      (select count(*) from public.catalog_taxonomy_nodes where publish_state = 'review')::integer as review_node_count,
  ...
  select jsonb_build_object(
    ...
    'review_node_count', m.review_node_count,
    ...
    'can_publish',
      ...
      and m.leaf_count = 51
      and m.review_node_count = m.node_count
      and m.published_node_count = 0
...
begin
  ...
  if p_expected_snapshot_hash is distinct from before_summary->>'snapshot_hash' then
    raise exception 'catalogue hierarchy or mapping changed; refresh and review again';
  end if;
  if not coalesce((before_summary->>'can_publish')::boolean, false) then
    raise exception 'complete the one-by-one owner review before publication';
  end if;

  -- Review evidence is preserved. This function never bulk-approves assignments.
  update public.catalog_taxonomy_nodes
  set publish_state = 'published',
      updated_by = actor,
      updated_at = now()
  where publish_state = 'review';
  ...
  set redirect_status = case when redirect_status = 'proposed' then 'approved' else redirect_status end,
      notes = case
        when source_kind = 'product'
          then 'Explicit hierarchy mapping owner-approved. Existing product URL remains available until a separately verified redirect release.'
        else 'Legacy category mapping owner-approved. Existing category URL remains available until a separately verified redirect release.'
      end,
      updated_at = now()
  ...
  snapshot := jsonb_build_object(
    ...
    'assignments_bulk_approved', false
  );
...
begin
  ...
  -- Keep individual product approvals as durable owner-review evidence.
  update public.catalog_taxonomy_migration_map
  ...
  insert into public.catalog_taxonomy_review_events(
    ...
    jsonb_build_object(
      'before', before_summary,
      'after', after_summary,
      'individual_assignment_approvals_preserved', true
    )
  );
...
comment on function public.catalog_publish_reviewed_taxonomy(text, integer, text) is
  'Authenticated owner/admin release action. Requires all 69 nodes in review, all 86 product mappings individually approved, exact count and immutable snapshot checks; records audit evidence and does not apply legacy redirects.';
comment on function public.catalog_unpublish_taxonomy(text) is
  'Authenticated reversible safety action that hides the explicit hierarchy while preserving individual product approval evidence.';
