...
rules as (
  select version, status, approved_at,
         rules #>> '{company,websiteState}' as website_state
...
select jsonb_build_object(
  ...
  'company', jsonb_build_object(
    ...
    'website_state', coalesce((select website_state from rules), 'Website age is not used as a buyer-trust claim'),
...
begin
  if exists (
    ...
    union all select 1 where lower(pg_get_functiondef('public.admin_ai_live_snapshot()'::regprocedure)) like '%newly built%'
