...
-- Applying this migration does not approve assignments or publish taxonomy nodes.
-- Publication is available only through an authenticated admin RPC with an exact
-- confirmation phrase, current-count preconditions and an immutable audit event.
...
comment on function public.catalog_publish_reviewed_taxonomy(text, integer, text) is
  'Authenticated owner/admin action. Publishes the explicit catalogue hierarchy only after exact count, snapshot and typed-confirmation checks; records audit evidence and does not apply legacy redirects.';
comment on function public.catalog_unpublish_taxonomy(text) is
  'Authenticated reversible safety action that returns the explicit catalogue hierarchy to review state.';
