...
-- Extends existing inquiry, catalogue lead and imported prospect tables without moving or deleting data.
...
-- Preserve the meaning of legacy imported-prospect statuses in the new unified CRM status.
...
DO $$ BEGIN
  ALTER TABLE public.catalogue_leads
    ADD CONSTRAINT catalogue_leads_priority_check
    CHECK (priority IN ('low', 'normal', 'high', 'urgent'));
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
  ALTER TABLE public.inquiries
    ADD CONSTRAINT inquiries_sample_status_check
...
DO $$ BEGIN
  ALTER TABLE public.catalogue_leads
    ADD CONSTRAINT catalogue_leads_sample_status_check
    CHECK (sample_status IN ('not_requested', 'requested', 'in_development', 'sent', 'approved', 'rejected'));
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
  ALTER TABLE public.b2b_leads
    ADD CONSTRAINT b2b_leads_sample_status_check
    CHECK (sample_status IN ('not_requested', 'requested', 'in_development', 'sent', 'approved', 'rejected'));
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
  ALTER TABLE public.b2b_leads
    ADD CONSTRAINT b2b_leads_crm_status_check
    CHECK (crm_status IN (
...
-- Existing catalogue_leads and b2b_leads updated_at triggers remain authoritative.
