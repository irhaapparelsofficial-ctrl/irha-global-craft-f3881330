...
type UploadRule = { contentType: string; aliases: string[] };

const TECH_PACK_UPLOADS: Record<string, UploadRule> = {
  pdf: { contentType: "application/pdf", aliases: ["application/pdf"] },
  ai: { contentType: "application/vnd.adobe.illustrator", aliases: ["", "application/octet-stream", "application/pdf", "application/postscript", "application/illustrator", "application/vnd.adobe.illustrator"] },
  eps: { contentType: "application/postscript", aliases: ["", "application/octet-stream", "application/postscript", "application/eps", "application/x-eps"] },
  zip: { contentType: "application/zip", aliases: ["", "application/octet-stream", "application/zip", "application/x-zip-compressed", "multipart/x-zip"] },
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!allowed) return json({ error: "Too many requests. Please try again later." }, 429, { ...headers, "Retry-After": "900" });
...
async function submitInquiry(service: ReturnType<typeof createClient>, payload: JsonRecord, headers: Record<string, string>) {
  ...
  if (["inquiry", "quote", "mockup"].includes(kind) && !email) return json({ error: "A valid email is required" }, 400, headers);
  if (kind === "inquiry" && (!company || !country)) return json({ error: "Company and country are required" }, 400, headers);

  const requestedRef = text(payload.inquiry_ref, 80);
  const inquiryRef = /^IRHA-[0-9]{4}-[0-9]{6}$/.test(requestedRef) ? requestedRef : createInquiryReference();
  const files = normalizeFiles(payload.files);
  const items = normalizeItems(payload.items);
  const incomingContext = safeObject(payload.lead_context, 50_000);
  const leadContext = {
    ...incomingContext,
    gateway: { kind, received_at: new Date().toISOString(), server_validated: true },
  ...
  if (items.length > 0) {
    const { data, error } = await service.rpc("submit_b2b_inquiry", {
      _payload: { ...payload, name, email, phone, company, country, category, message, source, inquiry_ref: inquiryRef, files, items, lead_context: leadContext },
    }).single();
    if (error) {
  ...
  const { data, error } = await service.from("inquiries").insert({
    ...
    tech_pack_paths: files.filter((file) => file.path.startsWith("requests/tech-pack/")).map((file) => file.path),
  }).select("inquiry_ref").single();

  if (error) {
    if (error.code === "23505" && error.message.toLowerCase().includes("inquiry_ref")) return json({ ok: true, reference: inquiryRef, duplicate: true }, 200, headers);
...
async function submitCatalogue(service: ReturnType<typeof createClient>, payload: JsonRecord, headers: Record<string, string>) {
  const name = text(payload.name, 100);
  const email = normalizeEmail(payload.email);
...
async function createUpload(service: ReturnType<typeof createClient>, payload: JsonRecord, headers: Record<string, string>) {
  ...
  if (!Number.isFinite(size) || size < 1 || size > maxBytes) return json({ error: `File must be between 1 byte and ${Math.round(maxBytes / 1024 / 1024)} MB` }, 400, headers);
  ...
  return json({ ok: true, bucket, path, token: data.token, content_type: rule.contentType, max_size: maxBytes }, 200, headers);
}
...
function normalizeFiles(value: unknown) {
  ...
  return value.slice(0, 5).flatMap((item) => {
    ...
    const validPath = path.startsWith("requests/tech-pack/") || path.startsWith("requests/mockup/") || path.startsWith("requests/inquiry/");
...
async function requestFingerprint(req: Request) {
  const ip = (req.headers.get("cf-connecting-ip") || req.headers.get("x-real-ip") || req.headers.get("x-forwarded-for")?.split(",")[0] || "unknown").trim();
...
function isAllowedOrigin(origin: string) {
  try {
    ...
    return url.hostname === "irhaapparels.com" || url.hostname === "www.irhaapparels.com" || url.hostname === "localhost" || url.hostname === "127.0.0.1" || url.hostname.endsWith(".lovable.app");
  } catch {
    return false;
  }
}
