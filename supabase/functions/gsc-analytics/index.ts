...
async function requireAdmin(req: Request, headers: Record<string, string>) {
  ...
  if (!/^Bearer\s+\S+/i.test(authHeader)) return { response: json({ error: "Unauthorized" }, 401, headers), userId: null };
  ...
  if (userError || !user) return { response: json({ error: "Unauthorized" }, 401, headers), userId: null };
  ...
  const { data: role, error: roleError } = await client
    ...
    .eq("role", "admin")
    .maybeSingle();
  if (roleError || !role) return { response: json({ error: "Admin only" }, 403, headers), userId: null };
  return { response: null, userId: user.id };
}

function connectionState() {
  const lovable = Boolean(Deno.env.get("LOVABLE_API_KEY"));
  const gsc = Boolean(Deno.env.get("GOOGLE_SEARCH_CONSOLE_API_KEY"));
  const siteUrl = (Deno.env.get("GSC_SITE_URL") || DEFAULT_SITE_URL).trim();
  return {
    ready: lovable && gsc,
    configuration: {
      connector_gateway_key: lovable,
      search_console_connection_key: gsc,
