...
import {
  ...
  getBavarianWomensCollection,
} from "@/lib/bavarianWomensCollections";
...
export default function BavarianWomensCollection() {
  const { collectionSlug = "" } = useParams<{ collectionSlug: string }>();
  const collection = getBavarianWomensCollection(collectionSlug);
  ...
  const productsBySlug = new Map<string, (typeof category.subs)[number]["products"][number]>();
  for (const subCategory of category.subs) {
    for (const product of subCategory.products) {
      if (collection.matches(product.slug, product.name) && !productsBySlug.has(product.slug)) {
  ...
  return (
    <>
      ...
      <nav aria-label="Women's Trachten collections" className="border-b border-border/60 bg-background">
        <div className="container-luxe flex gap-2 overflow-x-auto py-4">
          {BAVARIAN_WOMENS_COLLECTIONS.map((item) => (
            <Link
              key={item.slug}
              to={`/products/${TOP_CATEGORY}/womens-trachten/${item.slug}`}
