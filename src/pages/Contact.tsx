...
export default function Contact() {
  ...
  const socials = [
    { name: "Instagram", Icon: InstagramIcon, href: settings.socials.instagram, handle: "@irhaapparels" },
    { name: "Facebook", Icon: FacebookIcon, href: settings.socials.facebook, handle: settings.brand.name },
    { name: "LinkedIn", Icon: LinkedInIcon, href: "https://www.linkedin.com/company/irha-apparels", handle: settings.brand.name },
    { name: "TikTok", Icon: TikTokIcon, href: settings.socials.tiktok, handle: "@irhaapparels" },
    { name: "WhatsApp", Icon: WhatsAppIcon, href: whatsappHref, handle: settings.brand.phoneDisplay },
  ].filter((item) => Boolean(item.href));

  return (
    <>
      <SEO title={`Contact ${settings.brand.name} — ${settings.brand.location}`} description={`Reach ${settings.brand.name} via WhatsApp, email or direct call for international B2B apparel manufacturing enquiries.`} path="/contact" />
      ...
      <section className="py-24 md:py-32">
        <div className="container-luxe grid lg:grid-cols-2 gap-16">
          <div className="space-y-10">
            <ContactRow Icon={MessageCircle} label="WhatsApp (Preferred)" value={settings.brand.phoneDisplay} href={whatsappHref} cta="Open Chat" />
            <ContactRow Icon={Phone} label="Direct Line" value={settings.brand.phoneDisplay} href={`tel:${settings.brand.phone}`} cta="Call Now" />
            <ContactRow Icon={Mail} label="Email" value={settings.brand.email} href={`mailto:${settings.brand.email}`} cta="Send Email" />
            <ContactRow Icon={MapPin} label="Atelier" value={settings.brand.address} />
          </div>
          <div className="bg-card/40 border border-border p-10 md:p-12">
            ...
            <div className="space-y-4 text-foreground/75 mt-8"><div className="flex gap-3 border-b border-border/60 pb-4"><ShieldCheck size={18} className="text-gold shrink-0 mt-0.5" /><span>Use the Buyer Trust Center to review the verification process before ordering.</span></div><div className="flex gap-3 border-b border-border/60 pb-4"><Calendar size={18} className="text-gold shrink-0 mt-0.5" /><span>Factory video calls are scheduled after the category and preferred time window are reviewed.</span></div><div className="flex gap-3"><MessageCircle size={18} className="text-gold shrink-0 mt-0.5" /><span>WhatsApp is the fastest direct channel for urgent product or meeting context.</span></div></div>
            <div className="mt-10 flex flex-wrap gap-3"><a href={whatsappHref} target="_blank" rel="noreferrer noopener" className="inline-flex items-center gap-3 bg-gradient-gold text-primary-foreground px-7 py-4 text-xs uppercase tracking-[0.3em] hover:shadow-gold transition-all"><MessageCircle size={16} /> Start Conversation</a><Link to={settings.footer.factoryCallHref} className="inline-flex items-center gap-3 border border-foreground/25 hover:border-gold hover:text-gold px-7 py-4 text-xs uppercase tracking-[0.3em] transition-colors"><Calendar size={16} /> Factory Call</Link></div>
          </div>
        </div>
      </section>

      <section className="pb-24 md:pb-32 border-t border-border/60"><div className="container-luxe pt-16"><p className="eyebrow mb-8">Social</p><div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-8">{socials.map((social) => <a key={social.name} href={social.href} target="_blank" rel="noreferrer noopener" aria-label={`${settings.brand.name} on ${social.name}`} className="flex flex-col items-center gap-3 border border-border/60 p-6 hover:border-gold transition-colors group"><social.Icon className="text-foreground/60 group-hover:text-gold transition-colors" size={28} /><span className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground group-hover:text-foreground">{social.name}</span><span className="text-[9px] uppercase tracking-[0.15em] text-gold/80">{social.handle}</span></a>)}</div></div></section>
    </>
  );
}

function ContactRow({ Icon, label, value, href, cta }: { Icon: typeof Mail; label: string; value: string; href?: string; cta?: string }) {
  return <div className="border-b border-border pb-8"><div className="flex items-center gap-3 mb-3"><Icon className="text-primary" size={18} /><p className="text-[11px] uppercase tracking-[0.25em] text-muted-foreground">{label}</p></div><p className="font-display text-3xl md:text-4xl break-words">{value}</p>{href && cta && <a href={href} target="_blank" rel="noreferrer noopener" className="mt-4 inline-block text-xs uppercase tracking-[0.3em] text-primary hover-gold-underline">{cta} →</a>}</div>;
}
