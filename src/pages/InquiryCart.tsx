...
export default function InquiryCart() {
  ...
  const validate = () => {
    ...
    if (cart.items.length === 0) {
      ...
      if (incomplete) {
        nextErrors.items = `Enter a valid target quantity for ${incomplete.name}.`;
      }
    }
  ...
  if (result) {
    return (
      <>
        <SEO
          title="RFQ Received | Irha Apparels"
          description="Your B2B apparel manufacturing inquiry has been received."
          path="/inquiry-cart"
          noindex
        />
          ...
          <div className="container-luxe max-w-3xl">
            <div className="border border-primary/35 bg-card/50 p-8 text-center md:p-12">
              ...
              <div className="mt-8 flex flex-wrap justify-center gap-3">
                <Link to="/products" className="inline-flex min-h-12 items-center gap-2 bg-primary px-6 text-xs uppercase tracking-[0.22em] text-primary-foreground">
                  Browse more products
                </Link>
                <Link to="/factory-video-call" className="inline-flex min-h-12 items-center border border-border/70 px-6 text-xs uppercase tracking-[0.22em] hover:border-primary">
  ...
  return (
    <>
      <SEO
        title="Multi-Item B2B RFQ Cart | Irha Apparels"
        description="Build one structured multi-product RFQ with quantities, size breakdowns and private tech-pack uploads."
        path="/inquiry-cart"
        noindex
      />
      ...
      <section className="border-b border-border/60 pt-32 pb-10 md:pt-40">
        <div className="container-luxe">
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <h1 className="max-w-4xl font-display text-4xl leading-[1] md:text-6xl">
                One RFQ for every <span className="text-gold italic">selected style</span>.
              </h1>
              <p className="mt-5 max-w-2xl text-sm leading-7 text-foreground/65">
                No prices are shown. Add products from any collection, define target quantities and size requirements, then submit one sourcing request.
              </p>
            </div>
            <Link to="/products" className="inline-flex min-h-11 items-center gap-2 text-xs uppercase tracking-[0.22em] text-primary hover:text-primary/75">
          ...
          <div>
            <div className="mb-5 flex items-center justify-between gap-4">
              <div>
                ...
                <h2 className="mt-2 font-display text-3xl">{cart.count} style{cart.count === 1 ? "" : "s"}</h2>
              </div>
              {cart.count > 0 && (
                <button
                  type="button"
                  onClick={cart.clear}
                  className="inline-flex min-h-11 items-center gap-2 text-[10px] uppercase tracking-[0.2em] text-foreground/55 hover:text-destructive"
                >
            ...
            {errors.items && (
              <p className="mb-4 border border-destructive/40 bg-destructive/5 p-3 text-xs text-destructive" role="alert">
                {errors.items}
              </p>
            )}
            ...
            {cart.items.length === 0 ? (
              <div className="border border-dashed border-border/60 px-6 py-14 text-center">
                ...
                <p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-foreground/60">
                  Browse Bavarian wear, sportswear, streetwear, nightwear or leatherwear and select “Add to Inquiry.”
                </p>
                <Link to="/products" className="mt-6 inline-flex min-h-12 items-center bg-primary px-6 text-xs uppercase tracking-[0.22em] text-primary-foreground">
                  ...
                  <article key={item.slug} className="grid gap-5 border border-border/60 bg-card/35 p-4 sm:grid-cols-[112px_minmax(0,1fr)] md:p-5">
                    <Link to={inquiryCartProductPath(item)} className="relative aspect-[3/4] overflow-hidden bg-background/50">
                      {item.image ? (
                        <ThumbnailImage
                          src={item.image}
                          alt={`${item.name} custom wholesale manufacturing inquiry item`}
                          className="absolute inset-0 h-full w-full object-cover"
                        />
                      ) : (
                        <div className="absolute inset-0 grid place-items-center text-xs text-foreground/35">No image</div>
                      )}
                    </Link>
                    <div className="min-w-0">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <p className="text-[9px] uppercase tracking-[0.2em] text-foreground/45">
                            Style {index + 1}{item.categoryName ? ` · ${item.categoryName}` : ""}
                          </p>
                          <Link to={inquiryCartProductPath(item)} className="mt-1 block font-display text-xl hover:text-primary">
                            {item.name}
                          </Link>
                        </div>
                        <button
                          ...
                          className="inline-flex min-h-10 min-w-10 items-center justify-center text-foreground/45 hover:text-destructive"
                        >
                          <Trash2 size={15} />
                        </button>
                      </div>
                      ...
                      <div className="mt-5 grid gap-4 md:grid-cols-2">
                        <label className="block">
                          ...
                          <input
                            ...
                            onChange={(event) => cart.update(item.slug, { targetQuantity: event.target.value })}
                            placeholder="e.g. 500"
                            className="min-h-12 w-full border border-border/60 bg-input px-4 text-sm outline-none focus:border-primary"
                          />
                        ...
                        <label className="block">
                          ...
                          <input
                            ...
                            onChange={(event) => cart.update(item.slug, { sizeBreakdown: event.target.value })}
                            placeholder="e.g. S:100, M:200, L:150, XL:50"
                            className="min-h-12 w-full border border-border/60 bg-input px-4 text-sm outline-none focus:border-primary"
                          />
                        </label>
                      </div>
                      <label className="mt-4 block">
                        ...
                        <textarea
                          value={item.notes}
                          onChange={(event) => cart.update(item.slug, { notes: event.target.value })}
                          placeholder="Fabric, GSM, colourway, branding, packaging or sample requirements"
                          rows={2}
                          className="w-full resize-y border border-border/60 bg-input px-4 py-3 text-sm outline-none focus:border-primary"
                        />
                      </label>
                    </div>
                  </article>
          ...
          <aside className="h-fit border border-border/60 bg-card/45 p-6 xl:sticky xl:top-28 md:p-7">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h2 className="mt-1 font-display text-2xl">Submit RFQ</h2>
              </div>
            </div>

            <div className="mt-6 space-y-4">
              <Field label="Contact name *" error={errors.name}>
                <input value={form.name} onChange={(event) => updateForm("name", event.target.value)} autoComplete="name" className={inputClass} />
              </Field>
              <Field label="Company / brand *" error={errors.company}>
                <input value={form.company} onChange={(event) => updateForm("company", event.target.value)} autoComplete="organization" className={inputClass} />
              </Field>
              <Field label="Official business email *" error={errors.email}>
                <input type="email" value={form.email} onChange={(event) => updateForm("email", event.target.value)} autoComplete="email" placeholder="buyer@company.com" className={inputClass} />
              </Field>
              <Field label="Destination country *" error={errors.country}>
                <input value={form.country} onChange={(event) => updateForm("country", event.target.value)} autoComplete="country-name" placeholder="e.g. Germany" className={inputClass} />
              </Field>
              <Field label="WhatsApp / phone">
                <input value={form.phone} onChange={(event) => updateForm("phone", event.target.value)} autoComplete="tel" className={inputClass} />
              </Field>
              <Field label="Company size">
                <select value={form.companySize} onChange={(event) => updateForm("companySize", event.target.value as ContactForm["companySize"])} className={inputClass}>
              ...
              <Field label="General sourcing notes" error={errors.notes}>
                <textarea value={form.notes} onChange={(event) => updateForm("notes", event.target.value)} rows={4} className={`${inputClass} py-3`} placeholder="Target delivery, destination port, Incoterm or programme details" />
              </Field>
            </div>
            ...
            <button
              ...
              className="mt-7 inline-flex min-h-14 w-full items-center justify-center gap-3 bg-primary px-6 text-xs uppercase tracking-[0.24em] text-primary-foreground transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-45"
            >
              {submitting ? "Submitting securely…" : "Submit multi-item RFQ"} <Send size={15} />
            </button>
            <p className="mt-4 text-[10px] leading-5 text-foreground/45">
              Quotation-based B2B manufacturing only. No retail checkout or public pricing. Your files are stored in a private Supabase bucket.
            </p>
          </aside>
        </div>
      </section>
