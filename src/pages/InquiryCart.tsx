          ...
          <div className="container-luxe max-w-3xl">
            <div className="border border-primary/35 bg-card/50 p-8 text-center md:p-12">
              ...
              <div className="mt-8 flex flex-wrap justify-center gap-3">
                <Link to="/products" className="inline-flex min-h-12 items-center bg-primary px-6 text-xs uppercase tracking-[0.22em] text-primary-foreground">Browse more products</Link>
        ...
        <div className="container-luxe grid gap-8 xl:grid-cols-[minmax(0,1.35fr)_minmax(360px,.65fr)]">
          <div>
            <div className="mb-5 flex items-center justify-between gap-4">
              ...
              {cart.count > 0 && (
                <button type="button" onClick={cart.clear} className="inline-flex min-h-11 items-center gap-2 text-[10px] uppercase tracking-[0.2em] text-foreground/55 hover:text-destructive">
                ...
                {cart.items.map((item, index) => (
                  <article key={item.slug} className="grid gap-5 border border-border/60 bg-card/35 p-4 sm:grid-cols-[112px_minmax(0,1fr)] md:p-5">
                    <Link to={inquiryCartProductPath(item)} className="relative aspect-[3/4] overflow-hidden bg-background/50">
                      {item.image ? (
                        <ThumbnailImage src={item.image} alt={`${item.name} custom wholesale manufacturing inquiry item`} className="absolute inset-0 h-full w-full object-cover" />
                      ) : (
                    ...
                    <div className="min-w-0">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <button type="button" onClick={() => cart.remove(item.slug)} aria-label={`Remove ${item.name} from inquiry`} className="inline-flex min-h-10 min-w-10 items-center justify-center text-foreground/45 hover:text-destructive">
                          <Trash2 size={15} />
                        </button>
                      </div>
                      ...
                      <div className="mt-5 grid gap-4 md:grid-cols-2">
                        <label className="block">
                          ...
                          <input inputMode="numeric" value={item.targetQuantity} onChange={(event) => cart.update(item.slug, { targetQuantity: event.target.value })} placeholder="e.g. 500" className={inputClass} />
                        ...
                        <label className="block">
                          ...
                          <input value={item.sizeBreakdown} onChange={(event) => cart.update(item.slug, { sizeBreakdown: event.target.value })} placeholder="e.g. S:100, M:200, L:150, XL:50" className={inputClass} />
                        </label>
                      </div>
            ...
            <div className="mt-6 space-y-4">
              ...
              <Field label="Company size">
                <select value={form.companySize} onChange={(event) => updateForm("companySize", event.target.value as ContactForm["companySize"])} className={inputClass}>
                  <option value="">Select…</option><option value="1-10">1–10 employees</option><option value="11-50">11–50 employees</option><option value="51-200">51–200 employees</option><option value="201-500">201–500 employees</option><option value="501+">501+ employees</option>
                </select>
              </Field>
