...
type FunctionErrorLike = { message?: string; context?: { status?: number } };
...
async function invokeGateway(action: GatewayAction, payload: JsonRecord) {
  const { data, error } = await supabase.functions.invoke<GatewayResponse>("public-lead-gateway", { body: { action, payload } });
  if (error) {
...
function gatewayUnavailable(surface: "inquiry" | "catalogue" | "upload") {
  if (surface === "upload") return new Error("Secure file upload is temporarily unavailable. Submit without the file and share it through WhatsApp.");
  if (surface === "catalogue") return new Error("Secure catalogue request service is temporarily unavailable. Please retry or contact us through WhatsApp.");
...
export async function uploadPublicLeadFile(
  ...
  return { path: ticket.path, name: file.name, size: file.size, mime: contentType };
}
