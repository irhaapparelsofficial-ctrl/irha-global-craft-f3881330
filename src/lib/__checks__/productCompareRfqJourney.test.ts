...
describe("product inquiry cart and compare RFQ journey", () => {
  ...
  it("keeps full product identity in the persistent inquiry-cart handoff", () => {
    const product = read("src/pages/ProductDetail.tsx");
    expect(product).toContain("const savedProduct = {");
    expect(product).toContain("slug: product.slug");
    expect(product).toContain("name: product.name");
    expect(product).toContain("categorySlug: category.slug");
    expect(product).toContain("shortlist.toggle(savedProduct)");
    expect(product).toContain('to="/inquiry-cart"');
    expect(product).toContain("Upload tech pack in inquiry cart");
  });
