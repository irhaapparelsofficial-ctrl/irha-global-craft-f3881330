...
function sanitizeItem(value: unknown): InquiryCartItem | null {
  ...
  const image = sanitizeText(item.image, 2000);
  return {
    productId: sanitizeText(item.productId, 80) || undefined,
    slug,
    name,
    image: image ? thumbnailUrl(image) : undefined,
  ...
  if (current.length > 0) {
    ...
    if (migrated.length > 0) {
      try {
        ...
        // The current tab remains usable when persistent storage is unavailable.
      }
    }
  }
  hydrated = true;
...
export function updateInquiryItem(
  slug: string,
  patch: Partial<Pick<InquiryCartItem, "targetQuantity" | "sizeBreakdown" | "notes">>,
) {
  hydrate();
...
export function toggleInquiryItem(item: IncomingCartItem) {
  hydrate();
  if (snapshot.some((current) => current.slug === item.slug)) removeInquiryItem(item.slug);
  else addInquiryItem(item);
}
