...
import { Activity, AlertTriangle, Clock3, Play, RefreshCw, ShieldCheck } from "lucide-react";
...
export default function AutomationControlCenter() {
  ...
  const load = useCallback(async () => {
    ...
    const [settingsResult, runsResult, tasksResult, rulesResult] = await Promise.all([
      ...
      db.from("automation_tasks").select("id,module,action,title,status,requires_approval,external_action,error,created_at").order("created_at", { ascending: false }).limit(30),
  ...
  const taskCounts = useMemo(() => {
    ...
    snapshot.tasks.forEach((task) => {
      result[task.module] = (result[task.module] || 0) + 1;
    });
    return result;
  }, [snapshot.tasks]);

  const pending = snapshot.tasks.filter((task) => ["draft", "ready_for_review", "approved"].includes(task.status)).length;
  const failed = snapshot.tasks.filter((task) => task.status === "failed" || Boolean(task.error)).length;
  ...
  return (
    <section className="border border-border/60 bg-card/30 p-4 sm:p-6 space-y-5" aria-labelledby="automation-control-title">
      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-muted-foreground max-w-3xl leading-relaxed">
            Daily lead, SEO, listing, social and Canva planning. External sends, publications and commercial commitments remain approval-controlled.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <button
            type="button"
            onClick={() => void runPlanningCycle()}
            disabled={running || !snapshot.settings?.enabled}
            className="min-h-11 inline-flex items-center gap-2 bg-gradient-gold text-background px-4 py-2 text-[10px] uppercase tracking-[0.18em] disabled:opacity-50"
          >
      ...
      {error && (
        <div className="border border-destructive/50 bg-destructive/10 p-3 text-sm text-destructive flex items-start gap-2">
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        <div className="border border-border/50 p-4 space-y-3">
          ...
          {Object.keys(taskCounts).length === 0 ? (
            <p className="text-xs text-muted-foreground">No automation tasks yet.</p>
          ) : Object.entries(taskCounts).map(([module, count]) => (
            <Detail key={module} label={module} value={String(count)} />
          ))}
        </div>
      </div>
      ...
      <div className="grid xl:grid-cols-2 gap-4">
        <div className="border border-border/50 p-4">
          ...
          {snapshot.runs.length === 0 ? <Empty text="No planning run recorded." /> : (
              ...
              {snapshot.runs.map((run) => (
                <div key={run.id} className="border-b border-border/30 pb-2 last:border-0 text-xs flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="capitalize">{run.trigger_source} · {run.status}</p>
                    <p className="text-muted-foreground mt-1">{new Date(run.started_at).toLocaleString()}</p>
                  ...
                  <span className="shrink-0 text-[9px] uppercase tracking-[0.16em] text-muted-foreground">
                    external: {run.external_execution ? "yes" : "no"}
                  </span>
                </div>
              ))}
            </div>
        ...
        <div className="border border-border/50 p-4">
          <p className="text-xs uppercase tracking-[0.18em] mb-3">Latest task queue</p>
          {snapshot.tasks.length === 0 ? <Empty text="No tasks are waiting." /> : (
            <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
              {snapshot.tasks.slice(0, 12).map((task) => (
                <div key={task.id} className="border-b border-border/30 pb-2 last:border-0 text-xs">
                  <div className="flex items-start justify-between gap-3">
                    <p className="leading-relaxed">{task.title}</p>
                    <span className="shrink-0 text-[9px] uppercase tracking-[0.14em] text-gold">{task.status.replaceAll("_", " ")}</span>
                  </div>
                  <p className="mt-1 text-muted-foreground capitalize">
                    {task.module} · {task.action.replaceAll("_", " ")} · approval {task.requires_approval ? "required" : "not required"}
                  </p>
...
function StatusCard({ label, value, tone }: { label: string; value: string; tone: "good" | "warn" | "bad" }) {
  ...
  return (
    <div className="border border-border/50 p-4">
      ...
      <p className={`font-display text-xl mt-2 ${toneClass}`}>{value}</p>
    </div>
  );
}
...
function Detail({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start justify-between gap-3 text-xs border-b border-border/20 pb-2 last:border-0">
...
function Empty({ text }: { text: string }) {
  return <p className="text-xs text-muted-foreground py-4">{text}</p>;
}
