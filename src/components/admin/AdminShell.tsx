...
const ADVANCED_NAV: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { key: "chat", label: "AI Chat Records", description: "Review AI website-guide message records. Human Live Chat is in Plain Inbox.", icon: MessageSquare },
...
export function AdminShell({
  ...
  useEffect(() => {
    window.localStorage.setItem("irha-admin-advanced-open", String(advancedOpen));
  }, [advancedOpen]);
  ...
  const nav = (
    <nav className="flex-1 space-y-5 overflow-y-auto overscroll-contain px-2 py-4 pb-[max(1rem,env(safe-area-inset-bottom))]" aria-label="Admin sections">
      {PLAIN_NAV.map((group) => renderGroup(group))}
      <div className="border-t border-border/50 pt-4">
        <button
          type="button"
          onClick={() => setAdvancedOpen((current) => !current)}
          className={cn("flex min-h-12 w-full items-center gap-3 rounded-md px-3 text-left text-sm text-muted-foreground hover:bg-muted/40 hover:text-foreground", collapsed && "justify-center px-2")}
          aria-expanded={advancedOpen}
        >
          <Wrench size={17} className="shrink-0" />
          {!collapsed && <><span className="flex-1 font-medium">Advanced Tools</span>{advancedOpen ? <ChevronUp size={15} /> : <ChevronDown size={15} />}</>}
        </button>
    ...
    <div className="flex min-h-screen bg-background text-foreground">
      <aside id="admin-desktop-navigation" className={cn("sticky top-0 hidden h-screen flex-col border-r border-border/60 bg-card/40 transition-[width] duration-200 md:flex", collapsed ? "w-16" : "w-64")}>
        <div className={cn("flex min-h-16 items-center border-b border-border/60 px-4", collapsed && "justify-center px-0")}>
          {!collapsed ? <div className="min-w-0 flex-1"><p className="truncate font-display text-sm tracking-wide text-gold">IRHA ADMIN</p><p className="truncate text-[9px] uppercase tracking-[0.18em] text-muted-foreground">Plain Owner Mode</p></div> : <p className="font-display text-sm text-gold">IA</p>}
          <button type="button" onClick={() => setCollapsed((current) => !current)} className="inline-flex min-h-11 min-w-11 items-center justify-center text-muted-foreground hover:text-primary" aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"} aria-expanded={!collapsed} aria-controls="admin-desktop-navigation"><Menu size={16} /></button>
        </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-30 flex min-h-16 items-center gap-2 border-b border-border/60 bg-card/90 px-3 pt-[env(safe-area-inset-top)] backdrop-blur sm:gap-3 sm:px-4 md:px-6">
          <button type="button" className="-ml-2 inline-flex min-h-11 min-w-11 items-center justify-center text-foreground/80 md:hidden" onClick={() => setMobileOpen(true)} aria-label="Open menu" aria-expanded={mobileOpen} aria-controls="admin-mobile-navigation"><Menu size={20} /></button>
          <div className="min-w-0 flex-1 py-2"><p className="truncate text-[9px] uppercase tracking-[0.16em] text-muted-foreground sm:text-[10px] sm:tracking-[0.2em]">{groupTitle(view)}</p><h1 className="truncate font-display text-base leading-tight md:text-lg">{active?.label ?? "Dashboard"}</h1></div>
          <button type="button" onClick={() => setSearchOpen(true)} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-md border border-border/60 text-muted-foreground md:hidden" aria-label="Find an admin page"><Search size={17} /></button>
          ...
          <a href="/" target="_blank" rel="noopener noreferrer" className="hidden min-h-11 items-center gap-2 rounded-md border border-gold/60 px-3 py-2 text-[10px] uppercase tracking-[0.15em] text-gold transition-colors hover:bg-gold hover:text-background sm:inline-flex" title="Open live website"><ExternalLink size={12} /><span className="hidden xl:inline">View Website</span></a>
          <button type="button" onClick={signOut} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-md border border-border/60 px-2 py-2 text-[10px] uppercase tracking-[0.15em] hover:border-primary hover:text-primary sm:gap-2 sm:px-3" aria-label="Sign out"><LogOut size={13} /><span className="hidden xl:inline">Sign out</span></button>
        </header>

        <main id="admin-main" className="max-w-full flex-1 overflow-x-hidden p-3 pb-28 sm:p-4 sm:pb-28 md:p-6 md:pb-8 lg:p-8">
          {!plainView && <section className="mb-4 rounded-lg border border-border/60 bg-card/35 px-4 py-3 sm:mb-6" aria-label="Page guidance"><p className="text-sm font-medium">Advanced workspace</p><p className="mt-1 text-sm leading-relaxed text-muted-foreground">{active?.description ?? "Use this detailed tool only when needed."}</p></section>}
          {content}
        </main>
      </div>
