...
export default function WebsiteInquiriesPanel() {
  ...
  const load = useCallback(async () => {
    ...
    const [inq, cat] = await Promise.all([
      supabase
        .from("inquiries")
        .select("id, name, email, phone, company, country, category, quantity, message, status, created_at")
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      return [row.name, row.email, row.phone, row.company, row.country, row.categoryOrInterest, row.message]
