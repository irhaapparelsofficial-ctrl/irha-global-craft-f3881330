...
export default function GermanLanguageSuggestion() {
  ...
  const [suggestedLocale, setSuggestedLocale] = useState<Exclude<LocaleCode, "en"> | null>(null);

  useEffect(() => {
    if (getRouteLocale(pathname) !== "en" || getExplicitLanguagePreference() || isLanguageSuggestionDismissed()) {
    ...
    const languages = Array.isArray(navigator.languages) && navigator.languages.length > 0
      ? navigator.languages
      : [navigator.language];
    setSuggestedLocale(getSuggestedLocale(languages));
  }, [pathname]);

  if (!suggestedLocale) return null;

  const copy = SHARED_UI_COPY[suggestedLocale];
  const definition = LOCALE_REGISTRY[suggestedLocale];
  const close = () => { dismissLanguageSuggestion(); setSuggestedLocale(null); };
  const continueEnglish = () => { setExplicitLanguagePreference("en"); setSuggestedLocale(null); };

  return (
    <aside
      role="region"
      aria-label={copy.languageSuggestionLabel}
      data-irha-language-suggestion={suggestedLocale}
    ...
    >
      <p lang={suggestedLocale} className="pr-8 text-sm leading-6 text-foreground/82 md:flex-1 md:pr-0">{copy.languageSuggestionText}</p>
      <div className="mt-3 flex flex-wrap gap-2 md:mt-0 md:shrink-0">
        <a
          href={getLanguageDestination(pathname, suggestedLocale)}
          hrefLang={definition.hreflangCode}
          lang={suggestedLocale}
          onClick={() => setExplicitLanguagePreference(suggestedLocale)}
      ...
      <button type="button" aria-label={copy.dismissSuggestion} onClick={close} className="absolute right-2 top-2 inline-flex h-9 w-9 items-center justify-center rounded-full text-foreground/55 outline-none hover:bg-primary/10 hover:text-primary focus-visible:ring-2 focus-visible:ring-primary"><X size={17} /></button>
    </aside>
  );
}
