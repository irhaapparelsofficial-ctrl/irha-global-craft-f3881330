...
export default function Navbar() {
  ...
  const coreNav: ReadonlyArray<{ label: string; href: string; anchor?: boolean }> = [
    { label: copy.home, href: locale === "de" ? "/de/" : "/" },
    { label: copy.products, href: "/products" },
    { label: copy.manufacturing, href: "/manufacturing" },
    { label: copy.process, href: "/#process", anchor: true },
    { label: copy.buyerTrust, href: "/buyer-trust" },
  ];
  ...
  const quoteLabel = inquiryCount > 0 ? `${copy.reviewInquiry} (${inquiryCount})` : copy.requestQuote;
  ...
  return (
    <header className={cn("fixed inset-x-0 top-0 z-50 border-b transition-all duration-300", headerSolid ? "border-border/70 bg-background/96 py-2 shadow-[0_12px_35px_rgba(0,0,0,.22)] backdrop-blur-xl" : "border-transparent bg-gradient-to-b from-black/75 to-transparent py-2.5")}>
      <div className="container-luxe flex min-h-16 items-center justify-between gap-3">
        ...
        <nav className="hidden items-center gap-5 xl:flex" aria-label={locale === "de" ? "Hauptnavigation" : "Primary navigation"}>{coreNav.map((item) => item.anchor ? <a key={item.href} href={item.href} className="min-h-11 inline-flex items-center text-[9px] font-semibold uppercase tracking-[0.18em] text-foreground/72 transition-colors hover:text-primary">{item.label}</a> : <NavLink key={item.href} to={item.href} end={item.href === homeHref} className={({ isActive }) => cn("min-h-11 inline-flex items-center text-[9px] font-semibold uppercase tracking-[0.18em] transition-colors", isActive ? "text-primary" : "text-foreground/72 hover:text-primary")}>{item.label}</NavLink>)}</nav>
        ...
        <div className="hidden items-center gap-2 lg:flex">
          ...
          <Link to="/factory-video-call" className="hidden min-h-11 items-center px-3 text-[9px] font-semibold uppercase tracking-[0.16em] text-foreground/70 transition-colors hover:text-primary xl:inline-flex">{copy.factoryCall}</Link>
      ...
      {open && <nav id="mobile-navigation" aria-label={locale === "de" ? "Mobile Navigation" : "Mobile navigation"} className="container-luxe max-h-[calc(100dvh-76px)] overflow-y-auto pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-4 lg:hidden">
        <LanguageSelector mobile className="mb-3" />
        <div className="overflow-hidden rounded-xl border border-border/70 bg-card/95 shadow-elegant">{coreNav.map((item) => item.anchor ? <a key={item.href} href={item.href} className="flex min-h-14 items-center justify-between border-b border-border/60 px-5 text-sm font-medium text-foreground/85 last:border-b-0">{item.label}<ChevronRight size={16} className="text-primary" /></a> : <NavLink key={item.href} to={item.href} end={item.href === homeHref} className={({ isActive }) => cn("flex min-h-14 items-center justify-between border-b border-border/60 px-5 text-sm font-medium last:border-b-0", isActive ? "bg-primary/8 text-primary" : "text-foreground/85")}>{item.label}<ChevronRight size={16} className="text-primary" /></NavLink>)}</div>
        <div className="mt-3 grid grid-cols-2 gap-2"><Link to="/factory-video-call" className="inline-flex min-h-12 items-center justify-center rounded-md border border-border/70 px-3 text-[9px] font-semibold uppercase tracking-[0.16em] text-foreground/75">{copy.factoryCall}</Link><Link to="/inquiry-cart" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-md border border-border/70 px-3 text-[9px] font-semibold uppercase tracking-[0.16em] text-foreground/75"><ClipboardList size={14} /> {copy.inquiryCart} {inquiryCount || ""}</Link></div>
