        ...
        <div>
          ...
          {socials.length > 0 && (
            <div className="mt-5 flex gap-2">
              {socials.map((social) => (
                <a key={social.name} href={social.href} target="_blank" rel="noreferrer noopener" aria-label={`${settings.brand.name} on ${social.name}`} className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-foreground/15 text-foreground/60 transition-colors hover:border-primary hover:text-primary"><social.Icon /></a>
              ))}
            </div>
          )}
        </div>
      ...
      <div className="container-luxe mt-10 grid gap-5 border-t border-foreground/10 pt-6 md:grid-cols-[1fr_1.3fr]">
        <div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {INCOTERMS.map((term) => <span key={term} className="border border-foreground/15 px-3 py-2 text-[10px] uppercase tracking-[0.16em] text-foreground/58">{term}</span>)}
          </div>
          <p className="mt-3 text-[10px] leading-5 text-foreground/38">Final Incoterm, destination coverage, duties and commercial responsibility are confirmed in the written quotation.</p>
        </div>
        <div>
          <p className="text-[9px] font-semibold uppercase tracking-[0.2em] text-foreground/38">Data privacy & GDPR</p>
          <p className="mt-3 max-w-3xl text-[11px] leading-6 text-foreground/48">Inquiry data and private tech packs are collected for quotation, sampling and order communication. Buyers can review the privacy policy, manage non-essential cookies, or request access and deletion through the published contact details.</p>
        </div>
      </div>
