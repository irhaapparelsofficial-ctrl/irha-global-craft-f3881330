...
const base = "public/product-media/classic-bavarian-checkered-shirt/web";
...
describe("Classic Bavarian Checkered Shirt media link", () => {
  it("ships all four optimized product views", () => {
    for (const view of views) {
      const file = `${base}/classic-bavarian-checkered-shirt-design-01-${view}-web-1600.webp`;
      expect(existsSync(resolve(process.cwd(), file)), file).toBe(true);
      expect(migration).toContain(`https://irhaapparels.com/${file}`);
    }
  });
