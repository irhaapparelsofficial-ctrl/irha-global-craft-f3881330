...
describe("Irha CI control plane", () => {
  ...
  it("uses one append-only private manifest with exact Git blob checksums", () => {
    const database = read(".github/workflows/supabase-database-auto.yml");
    ...
    expect(manifest.migrations.length).toBeGreaterThanOrEqual(16);
    ...
    expect(manifest.migrations.map((migration: { version: string }) => migration.version)).toContain("20260717234000");
    expect(manifest.migrations.map((migration: { version: string }) => migration.version)).toContain("20260717234100");
