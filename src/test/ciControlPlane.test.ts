...
describe("Irha CI control plane", () => {
  ...
  it("uses one merged private manifest with exact Git blob checksums", () => {
    ...
    expect(new Set(manifest.migrations.map((migration: { version: string }) => migration.version)).size).toBe(17);
    expect(new Set(manifest.migrations.map((migration: { path: string }) => migration.path)).size).toBe(17);
