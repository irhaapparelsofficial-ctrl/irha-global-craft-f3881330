...
describe("Irha CI control plane", () => {
  ...
  it("syncs Supabase functions and reconciles the complete migration ledger only from exact green main", () => {
  ...
  it("preserves before, dry-run, after and post-apply parity evidence", () => {
    const database = read(".github/workflows/supabase-database-auto.yml");
    ...
    expect(database).toContain("migrations-before.txt");
    expect(database).toContain("migration-dry-run.txt");
    expect(database).toContain("migrations-after.txt");
