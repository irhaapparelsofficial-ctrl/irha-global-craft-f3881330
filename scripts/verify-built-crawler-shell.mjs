...
for (const file of htmlFiles) {
  ...
  const nodes = parseSchemas(page, relative(DIST, file)).flatMap((schema) => schemaNodes(schema));
  ...
  assert(organizations.length === 1, `${relative(DIST, file)} must contain exactly one Irha Apparels Organization node; found ${organizations.length}`);
  const organization = organizations[0];
  assert(organization["@id"] === PUBLIC_IDENTITY.organizationId, `${relative(DIST, file)} Organization @id drift`);
  assert(organization.name === PUBLIC_IDENTITY.name, `${relative(DIST, file)} Organization name drift`);
  assert(organization.url === PUBLIC_IDENTITY.url, `${relative(DIST, file)} Organization URL drift`);
  assert(organization.logo === PUBLIC_IDENTITY.logoUrl, `${relative(DIST, file)} Organization logo drift`);
  assert(organization.telephone === PUBLIC_IDENTITY.telephone, `${relative(DIST, file)} Organization telephone drift`);
  assert(organization.email === PUBLIC_IDENTITY.email, `${relative(DIST, file)} Organization email drift`);
  assert(organization.address?.addressLocality === PUBLIC_IDENTITY.address.locality, `${relative(DIST, file)} locality drift`);
  assert(organization.address?.addressRegion === PUBLIC_IDENTITY.address.region, `${relative(DIST, file)} region drift`);
  assert(organization.address?.addressCountry === PUBLIC_IDENTITY.address.country, `${relative(DIST, file)} country drift`);
  assert(JSON.stringify(organization.sameAs) === JSON.stringify(PUBLIC_IDENTITY.sameAs), `${relative(DIST, file)} sameAs drift`);
  for (const property of forbiddenProperties) assert(!(property in organization), `${relative(DIST, file)} emits forbidden Organization property ${property}`);
  assert(!nodes.some((node) => node["@type"] === "LocalBusiness" || (Array.isArray(node["@type"]) && node["@type"].includes("LocalBusiness"))), `${relative(DIST, file)} emits LocalBusiness`);
  const websites = nodes.filter((node) => node["@type"] === "WebSite" && node["@id"] === PUBLIC_IDENTITY.websiteId);
  assert(websites.length === 1, `${relative(DIST, file)} must contain exactly one canonical WebSite node`);
  assert(websites[0].publisher?.["@id"] === PUBLIC_IDENTITY.organizationId, `${relative(DIST, file)} WebSite publisher drift`);
  for (const product of nodes.filter((node) => node["@type"] === "Product")) {
    assert(product.manufacturer?.["@id"] === PUBLIC_IDENTITY.organizationId, `${relative(DIST, file)} Product manufacturer drift`);
  }
}
...
console.log(`PASS ${htmlFiles.length} built HTML files with one canonical Organization, one WebSite, ${sitemapCount} sitemap URLs, valid contact identity and preserved crawler contracts`);
