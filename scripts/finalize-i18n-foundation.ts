...
function buildGermanEntry(template: string): string {
  ...
  html = html.replace(/(<div\s+id=["']root["'][^>]*>)[\s\S]*?(<\/div>\s*<script\s+type=["']module["'])/i, `$1${selectorMarkup(page.path)}${germanGatewayShell()}$2`);
...
async function main() {
  ...
  console.log(`Internationalization foundation finalized: ${files.length} HTML shells, ${publishedGerman.length} published German routes`);
}
