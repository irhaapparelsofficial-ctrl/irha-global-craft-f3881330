...
async function inspectInitialHtml(pathname, expectedHref) {
  ...
  assert(canonicals.length === 1, `raw ${pathname}: expected one canonical, found ${canonicals.length}`);
  assert(canonicals[0]?.href === expectedHref, `raw ${pathname}: canonical ${canonicals[0]?.href}; expected ${expectedHref}`);
  assert(canonicals[0]?.fallback === true, `raw ${pathname}: static canonical is missing data-irha-fallback-seo=\"true\"`);
  return result;
}
...
async function assertSingleCanonical(page, expectedHref, label, phase) {
  const deadline = Date.now() + 6_000;
  let state = await captureCanonicalState(page, `${phase}:pre-assert`);
  while (Date.now() < deadline) {
    if (state.canonicalCount === 1 && state.canonicals[0]?.href === expectedHref) return state;
    await page.waitForTimeout(250);
    state = await captureCanonicalState(page, `${phase}:poll`);
  }
  const finalState = await captureCanonicalState(page, `${phase}:failure`);
  throw new Error(`${label}: expected exactly one canonical ${expectedHref}; actual=${JSON.stringify(finalState)}`);
}
...
async function waitForReactMount(page, phase) {
  try {
    await page.waitForFunction(() => {
      const root = document.querySelector('#root');
      return Boolean(root && !document.querySelector('#irha-app-boot-shell'));
    }, undefined, { timeout: 10_000 });
  } catch {
    // The following capture is intentionally authoritative even if the mount signal is absent.
  }
...
async function navigateByActualLink(page, path, label) {
  ...
  await captureCanonicalState(page, `${label}:after-spa-navigation`);
  await page.waitForTimeout(900);
  await captureCanonicalState(page, `${label}:stabilized-after-spa-navigation`);
  return assertSingleCanonical(page, `${CANONICAL_ORIGIN}${path}`, label, `${label}:canonical`);
}

async function main() {
  await verifyReleaseIdentity();
  await inspectInitialHtml('/', `${CANONICAL_ORIGIN}/`);
  await inspectInitialHtml(WATCH_PATH, `${CANONICAL_ORIGIN}${WATCH_PATH}`);
  await inspectInitialHtml(CALL_PATH, `${CANONICAL_ORIGIN}${CALL_PATH}`);
  for (const path of REPRESENTATIVE_PATHS) await inspectInitialHtml(path, `${CANONICAL_ORIGIN}${path}`);
  ...
  try {
    ...
    await captureCanonicalState(page, 'homepage:stabilized');
    await assertSingleCanonical(page, `${CANONICAL_ORIGIN}/`, 'homepage', 'homepage');
    ...
    for (const path of REPRESENTATIVE_PATHS) {
      ...
      await captureCanonicalState(page, `representative:${path}:after-spa-navigation`);
      await page.waitForTimeout(900);
      await assertSingleCanonical(page, `${CANONICAL_ORIGIN}${path}`, `representative ${path}`, `representative:${path}`);
    }
    await context.close();
  } finally {
    await browser.close();
