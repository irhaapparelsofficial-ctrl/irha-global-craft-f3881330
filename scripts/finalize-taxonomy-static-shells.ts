...
const legacyBlock = new RegExp(
  `\\s*<!-- taxonomy-legacy-shell-verification-only -->\\s*<url>\\s*<loc>${legacyUrl.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}<\\/loc>[\\s\\S]*?<\\/url>`,
  "g",
);
...
const plush = products.find((route) => route.productSlug === "plush-bathrobe-sleep-robe");
...
const legacyShell = readFileSync(resolve("dist", plush.legacyPath.slice(1), "index.html"), "utf8");
const expectedCanonical = `<link rel="canonical" href="https://irhaapparels.com${plush.canonicalPath}"`;
if (!canonicalShell.includes(expectedCanonical) || !legacyShell.includes(expectedCanonical)) {
